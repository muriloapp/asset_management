...
The weights violin plots illustrate the distribution of portfolio weights across resamples for each (resample scheme × portfolio) pair. Across both minvar and maxdiv, iid resampling consistently produces the widest distribution of weights. This indicates that weights are highly sensitive to random resampling of return rows. Block bootstrap, which retains temporal structure, and Gaussian bootstrap lead to slightly tighter weight distributions. Moreover, the violin plots of maxdiv, they tend to be taller and wider than those of minvar. This reflects that weights in maxdiv vary more across bootstrap samples. In minvar, portfolios often concentrate on a few low-volatility assets.
