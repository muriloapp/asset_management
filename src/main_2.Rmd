...
calculate_portfolio_se <- function(results_df) {
  results_df %>%
    filter(complete.cases(mean_return, vol, sharpe)) %>%
    group_by(portfolio) %>%
    summarise(
      across(c(mean_return, vol, sharpe),
...
if (exists("results_gauss") && is.data.frame(results_gauss) && nrow(results_gauss) > 0) {
  ...
  results_long_gauss <- results_gauss %>%
    dplyr::select(portfolio, mean_return, vol, sharpe) %>%
    dplyr::filter(complete.cases(mean_return, vol, sharpe)) %>%
    tidyr::pivot_longer(cols = c(mean_return, vol, sharpe),
                        names_to = "metric",
                        values_to = "value") %>%
    dplyr::mutate(metric = factor(metric, levels = c("mean_return", "vol", "sharpe"),
  ...
  scatter_data_g <- results_gauss %>% filter(complete.cases(vol, mean_return))
  if(nrow(scatter_data_g) > 0) {
    p2_g <- ggplot(scatter_data_g, aes(x = vol, y = mean_return, color = portfolio)) +
...
quant_t <- results_t %>%                             # original wide data
  dplyr::select(portfolio, mean_return, vol, sharpe) %>%     # keep the 3 metrics
  tidyr::pivot_longer(-portfolio,                            # long format
                      names_to  = "metric",
                      values_to = "value") %>%              
  dplyr::group_by(portfolio, metric) %>%                     # group by BOTH
...
if (exists("results_t") && is.data.frame(results_t) && nrow(results_t) > 0) {
  ...
  if (nrow(results_long_t) > 0) {
    ...
    plot_subtitle_density_t <- paste("Based on", B_sim, "simulations.", round(fit_t_check$nu, 2), ". Dashed lines are original point estimates.")
    ...
    plot_subtitle_boxplot_t <- paste("Boxes show distribution over", B_sim, "simulations.", round(fit_t_check$nu, 2), ". Diamonds are original point estimates.")
  ...
  plot_subtitle_scatter_t <- paste("Cloud shows", B_sim, "simulations.", round(fit_t_check$nu, 2), ". Diamonds are original point estimates.")
  
  scatter_data_t <- results_t %>% filter(complete.cases(vol, mean_return))
  if(nrow(scatter_data_t) > 0) {
    p2_t <- ggplot(scatter_data_t, aes(x = vol, y = mean_return, color = portfolio)) +
      geom_point(alpha = 0.3, size = 1.5) + # Simulation cloud
      scale_color_brewer(palette = "Set1", name = "Portfolio Strategy") +
